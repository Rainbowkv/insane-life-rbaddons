...
function FirstSetUp()
    ...
    if (Customize.Framework == "ESX" or Customize.Framework == "NewESX") then
        ...
        local money = Framework.Functions.GetPlayerData().money
        SendReactMessage('setMoney', money.cash.." | "..money.bank)
    end
...
-- rb_code
local visible = false  -- 这个需要和Customize.Display保持同步相等
RegisterCommand('ui_info', function()
    visible = not visible
    SendReactMessage('setFirstSetUp', {
        ID = GetPlayerServerId(PlayerId()),
        ServerName = Customize.ServerName,
...
AddEventHandler("QBCore:Player:SetPlayerData", function(data)
  SendReactMessage('setMoney', math.floor(data.money.cash).." | "..data.money.bank)
end)
