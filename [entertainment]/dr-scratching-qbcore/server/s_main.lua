...
RegisterNetEvent("dr-scratching:handler", function(returncooldown, cooldown)
  ...
  if returncooldown then
    if Config.ShowCooldownNotifications then
      TriggerClientEvent('QBCore:Notify', _source, '冷静一下，等一下再刮~', 'error')
    end
  ...
  if count >= 1 then
    ...
    if Config.ShowUsedTicketNotification then
      TriggerClientEvent('QBCore:Notify', _source, '成功使用了刮刮乐', 'success')
    end
  else
    ...
    if priceKey == key then
      ...
      if Config.ShowResultTicketNotification then
        if key == "Legendary" then
          ...
          -- 播放中奖音效（全城）
          TriggerClientEvent('QBCore:Notify', -1, '🌟 ' .. fullName .. ' 刮出了【传奇大奖】！全城震惊！', 'success', 10000)
          -- 播放中奖音效（仅该玩家）
          TriggerClientEvent('ds-scratching-qbcore:client:PlayWinSound', _source)
        end
