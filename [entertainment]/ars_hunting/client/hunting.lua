    ...
    CreateThread(function()
        while DoesEntityExist(prop) do
            ...
            if dist <= 55 then
                ...
                utils.drawText3D(vector3(coords.x, coords.y, coords.z - 0.5), "~o~ 诱饵", 1, 0)
            end
...
CreateThread(function()
    ...
    for _, zone in pairs(Config.HuntingZones) do
        for _, animal in pairs(zone.animals) do
            if not registeredModels[animal.model] then
                registeredModels[animal.model] = true

                exports.ox_target:addModel(animal.model, {
                    {
                        name = "harvest_animal",
                        label = locale('interact_haverest_animal'),
                        icon = 'fa-solid fa-knife',
