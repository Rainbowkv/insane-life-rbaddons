...
AddEventHandler('tj_restaurants:MakeDrink', function(data)
    ...
    elseif not washedHands then
        lib.notify({
            title = locale('notify_title'),
            description = locale('not_washed_hands'),
            type = 'error'
        })
        return
    end
...
AddEventHandler('tj_burgershot:duty', function()
    if onDuty then
        ...
        if savedOutfit then
            TriggerEvent('fivem-appearance:client:changeOutfit', savedOutfit)  -- 恢复服装
        end
        washedHands = false
        lib.notify({
            title = locale('notify_title'),
        ...
        TriggerServerEvent('QBCore:ToggleDuty')
        savedOutfit = {
            model = exports['fivem-appearance']:getPedModel(PlayerPedId()),
            components = exports['fivem-appearance']:getPedComponents(PlayerPedId()),
...
RegisterNUICallback("playSoundithinDistance", function(data, cb)
    local type = data.type

    if type == "placeOrder" then
        TriggerServerEvent('InteractSound_SV:PlayWithinDistance', 10, 'placeOrder', 0.3)
    end
