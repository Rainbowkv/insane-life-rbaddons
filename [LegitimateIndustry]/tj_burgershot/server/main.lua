...
lib.callback.register('tj_restaurants:checkItems', function(source, itemData)
    ...
    for _, v in pairs(itemData) do
        ...
        if Config.Inventory == 'ox' and Config.Framework == 'qb' then
        ...
        elseif Config.Inventory == 'qb' then
            ...
            totalCount = item and item.amount or 0
        elseif Config.Inventory == 'qs' then
            local hasItem = exports['qs-inventory']:HasItem(source, v.name)
            totalCount = hasItem and hasItem.count or 0
        elseif Config.Inventory == 'ox' and Config.Framework == 'esx' then
            local item = xPlayer.getInventoryItem(v.name)
            totalCount = item and item.count or 0
        end

        if totalCount < tonumber(v.amount) then
            return true
        end
    end
