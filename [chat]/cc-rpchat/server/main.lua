...
AddEventHandler('chatMessage', function(source, name, message)
    ...
    if ccChat:checkSpam(source, message) and config.antiSpam then
        TriggerClientEvent('cc-rpchat:addMessage', source, '#e67e22', 'fa-solid fa-triangle-exclamation', "请不要刷屏", '', false)
        return
    end
    if config.discord then
        playerName = "["..exports.ccDiscordWrapper:getPlayerDiscordHighestRole(source, "name").."] "..GetPlayerName(source)
    ...
    TriggerClientEvent('cc-rpchat:addProximityMessage', -1, '#f39c12', 'fa-solid fa-person', '我 | '..playerName, message, source, GetEntityCoords(GetPlayerPed(source)))  -- 默认改为me
    if config.DiscordWebhook then
