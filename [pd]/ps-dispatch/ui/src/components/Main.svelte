<script>
  ...
  function getDispatchData(dispatch) {
    if ($shortCalls) {
      return [
        { label: '报警', value: dispatch.data.message },
        { icon: 'fas fa-comment', label: '消息', value: dispatch.data.information },
      ];
    } else {
      return [
      { icon: 'fas fa-clock', label: '时间', value: timeAgo(dispatch.data.time) },
      { icon: 'fas fa-user', label: '市民', value: dispatch.data.name },
      { icon: 'fas fa-phone', label: '电话', value: dispatch.data.number },
      { icon: 'fas fa-comment', label: '消息', value: dispatch.data.information },
      { icon: 'fas fa-map-location-dot', label: '街道', value: dispatch.data.street },
      { icon: 'fas fa-user', label: '性别', value: dispatch.data.gender },
      { icon: 'fas fa-gun', label: '自动步枪射击', value: dispatch.data.automaticGunFire },
      { icon: 'fas fa-gun', label: '武器', value: dispatch.data.weapon },
      { icon: 'fas fa-car', label: '载具', value: dispatch.data.vehicle },
      { icon: 'fas fa-rectangle-list', label: '车牌', value: dispatch.data.plate },
      { icon: 'fas fa-droplet', label: '颜色', value: dispatch.data.color },
      { icon: 'fas fa-car', label: '类别', value: dispatch.data.class },
      { icon: 'fas fa-door-open', label: '车门', value: dispatch.data.doors },
      { icon: 'fas fa-compass', label: '方向', value: dispatch.data.heading },
      ];
    }
  }
</script>
