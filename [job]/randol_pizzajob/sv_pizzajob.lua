...
lib.callback.register('randol_pizzajob:server:Payment', function(source)
    ...
    if #players[src].locations == 0 then
        DoNotification(src, ('你收到了 $%s. 没有新的外卖订单了，请归还车辆.'):format(players[src].payment))
        return true
    end
    
    DoNotification(src, ('您收到了 $%s. 剩余订单数量: %s'):format(players[src].payment, #players[src].locations))
