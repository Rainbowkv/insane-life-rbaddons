...
local function TakePizza()
    ...
    if #(pos - vec3(currentDelivery.x, currentDelivery.y, currentDelivery.z)) >= 30.0 then
        return DoNotification('您离顾客房子太远了，先不急着取出披萨!', 'error')
    end
...
local function finishWork()
    ...
    --
    if oxtarget then
        exports.ox_target:removeEntity(NetworkGetNetworkIdFromEntity(pizzaCar), {'取出披萨', '放回披萨'})
    else
    ...
    if success then
        ...
        DoNotification('您结束了外卖兼职.', 'success')
        pizzaCar = nil
    end
end
...
local function yeetPed()
    if DoesEntityExist(pizzaBoss) then
        if oxtarget then
            exports.ox_target:removeLocalEntity(pizzaBoss, {'开始外卖工作', '结束外卖工作'})
        else
            exports['qb-target']:RemoveTargetEntity(pizzaBoss, {'Start Work', 'Finish Work'})
        end
        DeleteEntity(pizzaBoss)
...
local function changeOutfits()  -- rb_code, 穿上外卖服装
    local ped = PlayerPedId()
    local model = GetEntityModel(ped)
    local isMale = model == GetHashKey("mp_m_freemode_01")
    exports['fivem-appearance']:setPedProps(ped, {
        {
            prop_id = 0,
            drawable = isMale and 248 or 247,
            texture = 0
        }
    })
    exports['fivem-appearance']:setPedComponents(PlayerPedId(), {
        {
            component_id = 11, -- 上衣（夹克）
            drawable = isMale and 323 or 335,     -- 你要穿的夹克编号
            texture = 1        -- 材质编号
        },
        {
            component_id = 3,  -- 躯体（上半身）
            drawable = isMale and 0 or 0,
            texture = 0
        },
        {
            component_id = 4,  -- 下半身（裤子）
            drawable = isMale and 123 or 129,
            texture = 0
        },
        {
            component_id = 6,  -- 鞋子
            drawable = isMale and 70 or 73,
            texture = 0
        }
    })
end
...
local function deliverPizza()
    if holdingPizza and isHired and not pizzaDelivered then
        ...
        DoNotification('您需要先从车内取出披萨.', 'error') 
    end
end
...
function NextDelivery(data)
    ...
    activeOrder = true
    DoNotification('您有新的外卖配送订单了!', 'success')
end
