...
if framework == "ESX" then
    RegisterNetEvent("esx:setAccountMoney", function(account)
        ...
        Wait(50)
        TriggerServerEvent("ps-banking:server:logClient", account, moneyData)  -- 这个事件现在修改为logBankClient，这里已与esx不兼容
    end)
elseif framework == "QBCore" then
    RegisterNetEvent("QBCore:Client:OnMoneyChange", function(moneyType, amount, op)
        if moneyType == 'bank' then  -- 目前仅记录bank余额的日志，日后可以添加cash或自定义货币的日志
            TriggerServerEvent("ps-banking:server:logBankClient", amount, op)
        end
    end)
end
...
RegisterNUICallback("ps-banking:client:withdrawFromAccount", function(data, cb)
    if data.amount ~= math.floor(data.amount) then 
        QBCore.Functions.Notify("金额应当为整数", "error")
        return
    end
