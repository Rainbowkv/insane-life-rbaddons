...
RegisterNetEvent("ps-banking:server:logBankClient", function(amount, op)
    local src = source
    ...
    if op == 'add' then
        ...
        local isIncome = 1
        logTransaction(identifier, description, moneyType, amount, isIncome)
    else  -- 要么就是remove，qb-core里传过来的
        local description = "取出"
        local isIncome = 0
        logTransaction(identifier, description, moneyType, amount, isIncome)
    end
end)
...
-- rb_code
local function logAccountsTransaction(cardNumber, description, amount, isIncome, user_identifier)
    MySQL.insert.await('INSERT INTO ps_banking_accounts_transactions (cardNumber, description, amount, date, isIncome, user_identifier) VALUES (?, ?, ?, NOW(), ?, ?)', { cardNumber, description, amount, isIncome, user_identifier })
end
--
    ...
    if #account > 0 then
        ...
        if balance >= amount then
            ...
            if affectedRows > 0 then
                ...
                logAccountsTransaction(account[1].cardNumber, "取出", amount, 0, getPlayerIdentifier(xPlayer))  -- rb_code
                return true
            else
                return false
            end
...
lib.callback.register("ps-banking:server:depositToAccount", function(source, accountId, amount)
    ...
    local account = MySQL.query.await('SELECT * FROM ps_banking_accounts WHERE id = ?', { accountId })  -- rb_code
    if getPlayerAccounts(xPlayer) >= amount then
        ...
        if affectedRows > 0 then
            ...
            logAccountsTransaction(account[1].cardNumber, "存入", amount, 1, getPlayerIdentifier(xPlayer))  -- rb_code
            return true
        else
            return false
        end
