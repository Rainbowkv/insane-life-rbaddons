...
CreateThread(function()
    ...
    if Config.PedBlip then 
        local blip = AddBlipForCoord(Config.PedLocation)
    ...
    local npc = CreatePed(4, GetHashKey('a_m_m_rurmeth_01'), Config.PedLocation, 347.5, false, false)
    ...
    while true do
        ...
        if isLoaded then
            for k,v in pairs(Config.Graffitis) do
                ...
                if information then
                    if #(coords - v.coords) < Config.GraffitiRenderDistance then
                    ...
                    if information.blip == true then
                        if not DoesBlipExist(v.blip) then
                            v.blip = AddBlipForRadius(v.coords, Config.GraffitiBlipRadius)
                            SetBlipAlpha(v.blip, 100)
                            SetBlipColour(v.blip, information.blipcolor)
                            -- rb_code,添加中心点 blip
                            v.centerBlip = AddBlipForCoord(v.coords.x, v.coords.y, v.coords.z)
                            SetBlipSprite(v.centerBlip, 556) -- 自定义图标
                            SetBlipDisplay(v.centerBlip, 4)
                            SetBlipScale(v.centerBlip, 0.7)
                            SetBlipColour(v.centerBlip, information.blipcolor)
                            SetBlipAsShortRange(v.centerBlip, true)
                            BeginTextCommandSetBlipName("STRING")
                            AddTextComponentString(information.name)
                            EndTextCommandSetBlipName(v.centerBlip)
                        end
                    end
                end
            end
    ...
    if isLoaded then
        ...
        for k,v in pairs(Config.Graffitis) do
            if v then
                ...
                -- rb_code，移除点blip
                if DoesBlipExist(v.centerBlip) then
                    RemoveBlip(v.centerBlip)
                end
            end
        end
...
RegisterNetEvent('qb-graffiti:client:placeGraffiti', function(model, slot)
    ...
    if playerGang.name == 'none' then 
        QBCore.Functions.Notify("您没有帮派，无法进行领地涂鸦~", "error")
        return
    end
    if playerGang.grade.level < Config.AllowPaintGrade then
        QBCore.Functions.Notify("帮派的领头或者副手才能进行此操作~", "error")
        return
    end
    ----------
...
RegisterNetEvent('qb-graffiti:client:removeClosestGraffiti', function(slot)
    ...
    if playerGang.name == 'none' then 
        QBCore.Functions.Notify("您没有帮派，无法擦除领地涂鸦~", "error")
        return
    end
    if playerGang.grade.level < Config.AllowRemoveGrade then
        QBCore.Functions.Notify("您在帮派的等级过低，无法擦除其它帮派领地涂鸦~", "error")
        return
    end
    ----------
    local ped = PlayerPedId()
    local graffiti = GetClosestGraffiti(Config.RemoveDis)
